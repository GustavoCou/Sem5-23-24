...
export default class ThumbRaiser {
    ...
    constructor(generalParameters, mazeParameters, playerParameters, lightsParameters, fogParameters, fixedViewCameraParameters, firstPersonViewCameraParameters, thirdPersonViewCameraParameters, topViewCameraParameters, miniMapCameraParameters) {
        ...
        // Create the maze
        this.maze = new Maze(this.mazeParameters[this.mazeParameters.map]);
        ...
        //changes mapper state
        this.changeElevator = false;
        this.changeBridge = false;
    ...
    getPointedViewport(pointer) {
        let viewport;
        // Check if the pointer is over the mini-mapper camera viewport
    ...
    keyChange(event, state) {
        ...
        if (document.activeElement == document.body) {
            ...
            if (event.code == this.player.keyCodes.miniMap && state) { // Display / hide mini-mapper
    ...
    mouseDown(event) {
        if (event.buttons == 1 || event.buttons == 2) { // Primary or secondary button down
            ...
            if (cameraView != "none") {
                if (cameraView == "mini-mapper") { // Mini-mapper camera selected
    ...
    mouseWheel(event) {
        ...
        // Select the camera whose view is being pointed
        const cameraView = this.getPointedViewport(this.mousePosition);
        if (cameraView != "none" && cameraView != "mini-mapper") { // One of the remaining cameras selected
            const cameraIndex = ["fixed", "first-person", "third-person", "top"].indexOf(cameraView);
            this.view.options.selectedIndex = cameraIndex;
    ...
    collision(position) {
        ...
        return this.maze.distanceToWestWall(position) < this.player.radius || this.maze.distanceToEastWall(position) < this.player.radius || this.maze.distanceToNorthWall(position) < this.player.radius || this.maze.distanceToSouthWall(position) < this.player.radius || this.maze.distanceToWestWallwithoutDoor(position) < this.player.radius || this.maze.distanceToEastWallwithoutDoor(position) < this.player.radius || this.maze.distanceToNorthWallWithoutDoor(position) < this.player.radius || this.maze.distanceToSouthWallWithoutDoor(position) < this.player.radius || this.maze.distanceElevatorWestWall(position) < this.player.radius || this.maze.distanceElevatorEastWall(position) < this.player.radius || this.maze.distanceElevatorNorthWall(position) < this.player.radius || this.maze.distanceElevatorSouthWall(position) < this.player.radius;

    }

    enterElevator(newPosition, previousPosition) {
        return this.maze.enterElevator(newPosition, previousPosition);

    }

    enterBridge(newPosition) {

        let bridgePoint = this.maze.enterBridge(newPosition);

        if (bridgePoint !== Infinity) {
            this.changeMapBridge(bridgePoint);
        }

    }


    changeMap(selectedFloor) {

        this.gameRunning = false;
        this.scene3D.clear();
        this.maze = new Maze(mazeData[selectedFloor]);
        this.selectedFloorFilePath = selectedFloor;
    };

    changeMapElevator(selectedFloor) {
        console.log("floor q vem d tras:", selectedFloor);

        this.changeElevator = true;
        this.gameRunning = false;
        this.selectedFloorFilePath = selectedFloor;

        this.scene3D.clear();
        console.log("antes:", this.maze.elevatorExit);

        this.maze = new Maze(mazeData[selectedFloor]);

        console.log("depois:", this.maze.elevatorExit);



    }

    changeMapBridge(bridgePoint) {

         console.log(" next filepath "+ bridgePoint )
        this.changeBridge = true;
        this.gameRunning = false;

        this.scene3D.clear();

        this.maze = new Maze(merge({},mazeData[bridgePoint.filePath],{initialPositionBridge: bridgePoint.exitCell },{initialDirectionBridge: bridgePoint.exitDirection}));
    ...
    update() {
        if (!this.gameRunning) {
            ...
            if (this.maze.loaded && this.player.loaded) { // If all resources have been loaded
                ...
                if (this.changeElevator == true) {

                    this.player.position = this.maze.elevatorExit.clone();
                    this.player.direction = this.maze.initialDirection;

                    this.changeElevator = false;
                } else if (this.changeBridge) {

                    this.player.position =this.maze.initialPositionBridge;
                    this.player.direction = this.maze.initialDirectionBridge
                ...
                // Create the user interface if it not exit
                ...
                if (this.maze.foundExit(this.player.position)) {
                    ...
                    if (this.player.keyStates.backward) {
                        ...
                        if (this.collision(newPosition)) {
                            ...
                            this.player.position = newPosition;
                            if (this.enterElevator(newPosition, previousPosition)) {
                                console.log("entrei no if enters elevator, yey");
            ...
            // Render secondary viewport (mini-mapper)
            if (this.miniMapCheckBox.checked) {
                this.scene3D.fog = null;
    ...
    showElevatorPopUp() {
        /*let elevatorFloors = this.maze.elevatorFloors;
        console.log("elevator floors  ", elevatorFloors);
        const floorNumber = elevatorFloors.mapper(function (task) {
            return task.piso;
        });
        console.log("show elevator pop up ", floorNumber);


*/
        console.log(document.getElementById("floorModal").Id);
        document.getElementById("floorModal").innerHTML = "";
        this.popUpModal.style.display = "block";
        let elevatorFloors = this.maze.elevatorFloors;
        console.log("elevator floors  ", elevatorFloors);
        var tree = document.createDocumentFragment();
        var link = document.createElement("option");
        link.setAttribute("id", "noSelection");
        link.setAttribute("value", 0);
        link.appendChild(document.createTextNode("Not selected"));

        tree.appendChild(link);
        document.getElementById("floorModal").appendChild(tree);

        elevatorFloors.forEach(task => {
            var tree = document.createDocumentFragment();
            var link = document.createElement("option");
            link.setAttribute("id", "piso" + task.piso);
            link.setAttribute("value", task.filePath);

            link.appendChild(document.createTextNode(task.nome));

            tree.appendChild(link);
            document.getElementById("floorModal").appendChild(tree);
        });

        //   this.changeMapElevator(task.filePath);
