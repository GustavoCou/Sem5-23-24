...
*/
    ...
    updateElevatorFloorsDropdown(elevatorFloors) {
        const dropdown = document.getElementById("elevator-floor");
        dropdown.innerHTML = ''; // Clear existing options
        elevatorFloors.forEach(floorUrl => {
            const option = document.createElement("option");
            option.value = floorUrl.split('/').pop().split('.')[0]; // Extract floor number
            option.text = `Floor ${option.value}`;
