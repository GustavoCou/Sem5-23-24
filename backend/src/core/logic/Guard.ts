...
export class Guard {
  ...
  public static greaterThanOrEqual(value: number, minValue: number, argumentName: string): IGuardResult {
    if (value >= minValue) {
        return { succeeded: true };
    } else {
        return { succeeded: false, message: `${argumentName} must be greater than or equal to ${minValue}` };
    }
  }
}